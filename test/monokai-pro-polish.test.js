...
test("核心阅读视觉降低噪音并保留 Monokai 语义", () => {
  ...
  assert.match(base, /--h4-color:\s*#\{\$color-light-green\};/);
  assert.match(base, /--h5-color:\s*#\{\$color-light-cyan\};/);
  assert.match(base, /--h6-color:\s*#\{\$color-light-purple\};/);
  ...
  assert.match(editor, /--monokai-bold-italic-color:\s*var\(--bold-color\);/);
  assert.match(editor, /strong em,[\s\S]*?em strong[\s\S]*?color:\s*var\(--monokai-bold-italic-color\);/);
  assert.match(editor, /\.cm-strong\.cm-em,[\s\S]*?color:\s*var\(--monokai-bold-italic-color\);/);
  assert.match(typographySettings, /id: monokai-syntax-color-bold[\s\S]*?type: class-toggle/);
  assert.match(typographySettings, /id: monokai-syntax-color-italic[\s\S]*?type: class-toggle/);
  assert.match(typographySettings, /id: monokai-syntax-color-bold-italic[\s\S]*?type: class-toggle/);
  assert.match(styleSettingsScss, /body\.theme-dark\.monokai-syntax-color-bold[\s\S]*?--bold-color:\s*#\{\$color-pro-magenta\};/);
  assert.match(styleSettingsScss, /body\.theme-light\.monokai-syntax-color-italic[\s\S]*?--italic-color:\s*#\{\$color-light-code-purple\};/);
  assert.match(styleSettingsScss, /body\.theme-dark\.monokai-syntax-color-bold-italic[\s\S]*?--monokai-bold-italic-color:\s*#\{\$color-pro-orange\};/);
