...
const checks = [
  ...
  [
    ...
    /--monokai-table-api-name-color:\s*var\(--monokai-spectrum-cyan, var\(--link-color\)\);/.test(files.editor)
      ...
      && /--monokai-table-max-width:\s*(?:min\(100%, 62rem\)|100%);/.test(files.editor)
      && /--monokai-table-min-width:\s*(?:min\(100%, 48rem\)|fit-content|0);/.test(files.editor)
      ...
      && /\.markdown-rendered[\s\S]*?table\s*\{[\s\S]*?border:\s*var\(--monokai-table-border-width\) solid var\(--monokai-table-border\);[\s\S]*?line-height:\s*var\(--monokai-table-line-height\);[\s\S]*?margin-inline:\s*0;[\s\S]*?max-width:\s*(?:100%|none);[\s\S]*?min-width:\s*(?:var\(--monokai-table-min-width\)|fit-content|0);[\s\S]*?table-layout:\s*auto;[\s\S]*?width:\s*100%;/.test(files.editor)
      ...
      && /\.markdown-rendered[\s\S]*?th:first-child,[\s\S]*?td:first-child\s*\{[\s\S]*?min-width:\s*(?:5\.5rem|0);[\s\S]*?white-space:\s*(?:nowrap|normal);/.test(files.editor)
  ...
  [
    ...
    /--monokai-codeblock-active-line-background:\s*var\(--background-modifier-hover\);/.test(files.editor)
      ...
      && /--monokai-codeblock-selection-background:\s*(?:rgb\(120 220 232 \/ 32%\)|var\(--monokai-selection-background)/.test(files.editor)
